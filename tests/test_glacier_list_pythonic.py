"""Tests for the new Pythonic methods added to GlacierList."""
...
class TestGlacierListPythonic:
    """Test class for new Pythonic methods."""

    def setup_method(self):
        """Set up test fixtures."""
        self.temp_dir = tempfile.mkdtemp()
        self.cl = GlacierList(chunk_size=3, disk_path=self.temp_dir, auto_cleanup=False)
    ...
    def test_str_and_repr(self):
        ...
        # Test empty list
        assert str(self.cl) == "GlacierList(empty)"
        assert "GlacierList:" in repr(self.cl)
        assert "0 items" in repr(self.cl)

        # Test non-empty list
        self.cl.append({"id": 1, "value": "test"})
        assert str(self.cl) == "GlacierList(1 items)"
    ...
    def test_configuration_validation(self):
        ...
        with pytest.raises(ValueError, match="chunk_size must be positive"):
            GlacierList(chunk_size=-1)

        # Test valid configurations
        cl = GlacierList(chunk_size=1, disk_path="test", auto_cleanup=False)
...
class TestGlacierListPythonicIntegration:
    ...
    def test_method_chaining_simulation(self):
        ...
        try:
            with GlacierList(
                chunk_size=3, disk_path=temp_dir, auto_cleanup=False
            ) as cl:
                # Build a list
                items = [{"id": i, "value": f"test_{i}"} for i in range(8)]
                cl.extend(items)
