...
GlacierList provides a high-performance, memory-efficient alternative to Python lists
...
class GlacierList:
    ...
    When the list gets too large, data is chunked into `.pkl` files in the `data/` directory.
    When retrieving slices, only relevant chunks are loaded into memory.
    """
    ...
    def __init__(
        ...
        """Initialize a GlacierList with configurable parameters.
    ...
    def __getitem__(
        ...
        if isinstance(index, int):  # Single index lookup
            ...
            for idx in range(start, stop, step if step else 1):
                results.append(
                    self[idx]
                )  # Use recursive call to handle memory/disk logic
    ...
    def serialize(self, max_workers: Optional[int] = None):
    ...
    def map(self, func, max_workers: Optional[int] = None):
    ...
    def __repr__(self) -> str:
        """Return a string representation of the GlacierList."""
        return f"<GlacierList: {len(self)} items across {self.chunk_count} chunks>"

    def __str__(self) -> str:
        """Return a user-friendly string representation."""
        if self.is_empty:
            return "GlacierList(empty)"
        return f"GlacierList({len(self)} items)"

    def __eq__(self, other) -> bool:
        """Check equality with another GlacierList or list."""
        if isinstance(other, GlacierList):
    ...
    def remove(self, value: Dict[str, Any]):
    ...
    def copy(self) -> "GlacierList":
        """Return a shallow copy of the list."""
        new_list = GlacierList(chunk_size=self.chunk_size, disk_path=self.disk_path)
        new_list.extend(self)
        return new_list


def append_data(cl: GlacierList, num_items: int) -> None:
    """Appends a specified number of items to the GlacierList.

    Args:
        cl (GlacierList): The GlacierList instance to append data to.
...
def example_usage():
    """Demonstrate the usage of GlacierList."""
    # Create the disk-backed list
    cl = GlacierList(chunk_size=50_000)
    ...
    # Test to ensure no instance of list, bool, or dict within the values from cl[9_000:11_000]
    ...
    print(
        f"The length of the list is {len(cl)} and the length of the list converted to a list is {len(list(cl))}"
    )
